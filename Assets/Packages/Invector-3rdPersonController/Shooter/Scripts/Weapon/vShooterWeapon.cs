    ...
    public class vShooterWeapon : vMonoBehaviour
    {
        ...
        protected virtual IEnumerator ShootBullet(GameObject bullet, Vector3 dir)
        {
            ...
            try
            {
                ...
                RaycastHit hit;
                if (Physics.Raycast(transform.position, _camera.transform.forward, out hit, 15f))
                {
                    hit.collider.GetComponent<vHealthController>().TakeDamage(_damage);
                }
            }
            catch
            {
